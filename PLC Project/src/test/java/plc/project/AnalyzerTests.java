...
public final class AnalyzerTests {
    ...
    public void testSource(String test, Ast.Source ast, Ast.Source expected) {
        ...
        if (expected != null) {
            expected.getFields().forEach(field -> Assertions.assertEquals(field, analyzer.scope.lookupVariable(field.getName())));
    ...
    private static Stream<Arguments> testSource() {
        return Stream.of(
                ...
                Arguments.of("Invalid Return",
                        new Ast.Source(
                                Arrays.asList(
                                        new Ast.Field("value", "Boolean", Optional.of(new Ast.Expr.Literal(true)))
                                ),
                                Arrays.asList(
                                        new Ast.Method("main", Arrays.asList(), Arrays.asList(), Optional.of("Integer"), Arrays.asList(
                                                new Ast.Stmt.Return(new Ast.Expr.Access(Optional.empty(), "value")))
                                        )
                                )
                        ),
                        null
